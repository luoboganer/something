 ...
 * @Desc: A implementation of basic CNN with MINIST dataset
"""
...
conv1 = mx.symbol.Convolution(
    data=data, kernel=(5, 5), num_filter=32, name="conv1")
bn1 = mx.symbol.BatchNorm(data=conv1, name="bn1", fix_gamma=False)
act1 = mx.symbol.Activation(data=bn1, act_type="relu", name="act1")
pool1 = mx.symbol.Pooling(
    data=act1, kernel=(3, 3), stride=(2, 2), name="pool1", pool_type="max"
)

conv2 = mx.symbol.Convolution(
    data=pool1, kernel=(5, 5), num_filter=64, name="conv2")
bn2 = mx.symbol.BatchNorm(data=conv2, name="bn2", fix_gamma=False)
act2 = mx.symbol.Activation(data=bn2, act_type="relu", name="act2")
pool2 = mx.symbol.Pooling(
    data=act2, pool_type="max", kernel=(3, 3), stride=(2, 2), name="pool2"
)

conv3 = mx.symbol.Convolution(
    data=pool2, name="conv3", kernel=(1, 1), num_filter=10)
pool3 = mx.symbol.Pooling(
    data=conv3, name="pool3", global_pool=True, pool_type="avg", kernel=(1, 1)
)
...
result=zip(out.argmax(axis=1),test_iter.getlabel()[0].asnumpy(),out.max(axis=1))
for item in result:
    print(item)
...
while True:
    ...
    for i in range(len(out)):
        if out[i]!=test_iter.getlabel()[0].asnumpy()[i]:
            print("predicted:{},label:{}".format(out[i],test_iter.getlabel()[0].asnumpy()[i]))
            plt.imshow(test_iter.getdata()[0].asnumpy()[i].reshape(28,28),cmap="Greys_r")
            plt.axis("off")
            plt.show()
            flag=True
            break
...
my_batchData=my_batch([mx.nd.array(test_img[0].reshape(1,1,28,28).astype(np.float32))],None)
model.forward(my_batchData)
np.set_printoptions(suppress=True)
print("predicted probality:",model.get_outputs()[0].asnumpy())
print("predicted_result:{}".format(model.get_outputs()[0].asnumpy().argmax(axis=1)))
